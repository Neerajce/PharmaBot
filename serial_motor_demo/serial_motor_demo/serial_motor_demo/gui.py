...
class MotorGui(Node):
    ...
    def __init__(self):
        ...
        self.create_timer(0.01,self.do_stuff)

    def do_stuff(self):
        self.countr = self.countr + 1
        print('self.countr is ',self.countr)
        if self.countr == 200:
            mtr_cmd = MotorCommand()
            mtr_cmd.mot_1_req_rad_sec = 100.0
            mtr_cmd.mot_2_req_rad_sec = 100.0
            mtr_cmd.is_pwm = True
            self.countr = 0
        else:
            mtr_cmd = MotorCommand()
            mtr_cmd.mot_1_req_rad_sec = 200.0
            mtr_cmd.mot_2_req_rad_sec = 200.0
            mtr_cmd.is_pwm = True
        self.publisher.publish(mtr_cmd)

    def encoder_val_callback(self,msg):
        print(msg.mot_1_enc_val)
        print(msg.mot_2_enc_val)
